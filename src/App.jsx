...
function SetupPage({user,onDone,onCancel,existingTasks,T}){
  ...
  const save=async()=>{
    ...
    await supabase.from("users").update({tasks:cleaned}).eq("id",user.id);
    await supabase.from("admin_logs").insert({user_id:user.id,action:"subjects_updated",data:{subjects:cleaned.map(t=>t.label).join(", "),count:cleaned.length}}).catch(()=>{});
    onDone(cleaned);setSaving(false);
  };
...
function SettingsPage({user,userData,onUpdateTasks,onLogout,onDeleteAccount,onRefresh,isDark,toggleTheme,T}){
  ...
  return(
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      {showDelete&&<ConfirmModal title="Request Account Deletion" desc={`Your deletion request will be sent to the admin. You'll be logged out now. The admin will remove your account manually.\n\nUsername: ${userData?.username}\nEmail: ${user.email}`} confirmLabel="Yes, Send Request & Log Out" isDelete={true} onConfirm={()=>{setShowDelete(false);onDeleteAccount();}} onCancel={()=>setShowDelete(false)}/>}
    </div>
  );
}
...
// ─── ADMIN PAGE ────────────────────────────────────────────────────────────
const ACTION_ICONS={"signup":"🆕","login":"🔑","session_complete":"⏱","subjects_updated":"📚","username_changed":"✏️","avatar_changed":"🎭","streak_update":"🔥","logout":"🚪","account_deleted":"🗑️","delete_requested":"🚨","setup_subjects":"📝"};
const ACTION_COLORS={"signup":"#10b981","login":"#6366f1","session_complete":"#a855f7","subjects_updated":"#f59e0b","username_changed":"#06b6d4","avatar_changed":"#ec4899","streak_update":"#f97316","logout":"#9ca3af","account_deleted":"#ef4444","delete_requested":"#ef4444","setup_subjects":"#10b981"};
    ...
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      {!loading&&tab==="users"&&(()=>{
        ...
        const inactive=users.filter(u=>{
          ...
          if(!last)return(now-new Date(u.created_at).getTime())>oneYear;
          return(now-new Date(last+"T00:00:00").getTime())>oneYear;
        });
        return(
          <>
            {inactive.length>0&&(
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:"10px",color:T.dim}}>
                        ...
                        {!lastDate&&<span style={{marginLeft:"8px",color:"#f87171"}}> · Never studied</span>}
                      </div>
                    </div>
                    <div style={{textAlign:"right"}}>
            ...
            <div style={{padding:"12px 14px",background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.4)",borderRadius:"12px",marginBottom:"14px"}}>
              ...
              {logs.filter(l=>l.action==="delete_requested").map(l=>(
                <div key={l.id} style={{padding:"8px 10px",background:"rgba(239,68,68,0.1)",borderRadius:"8px",marginBottom:"6px"}}>
                  ...
                  <div style={{fontSize:"10px",color:"#f87171",marginTop:"4px",fontStyle:"italic"}}>→ Remove manually: Supabase → users table + Authentication</div>
                </div>
              ))}
            </div>
          )}
          ...
          {logs.map(l=>{
            ...
            return(
              <div key={l.id} style={{padding:"11px 12px",background:isSession?"rgba(168,85,247,0.06)":"rgba(245,158,11,0.04)",borderRadius:"10px",border:`1px solid ${isSession?"rgba(168,85,247,0.15)":"rgba(245,158,11,0.12)"}`,marginBottom:"7px"}}>
                ...
                {/* Row 3: Non-session data summary */}
                {!isSession&&l.data&&(
                  <div style={{fontSize:"10px",color:T.dim,lineHeight:1.7}}>
                    {Object.entries(l.data).slice(0,5).map(([k,v])=>(
...
export default function App(){
  ...
  const loadUserData=async(uid)=>{const {data}=await supabase.from("users").select("*").eq("id",uid).single();setUserData(data);if(data?.theme)setIsDark(data.theme!=="light");return data;};
  const loadSessions=async(uid)=>{const {data}=await supabase.from("sessions").select("*").eq("user_id",uid).order("date",{ascending:false});setSessions(data||[]);};
  ...
  useEffect(()=>{
    ...
    const interval=setInterval(()=>loadAll(uid),2000);
  ...
  useEffect(()=>{
    const init=async()=>{
      ...
      if(session){
        ...
        if(!ud?.tasks?.length)setShowSetup(true);setDataLoading(false);
      }
      setAuthLoading(false);
    };
    init();
    const {data:{subscription}}=supabase.auth.onAuthStateChange(async(ev)=>{if(ev==="SIGNED_OUT"){setAuthUser(null);setUserData(null);setSessions([]);}});
    return()=>subscription.unsubscribe();
  },[]);

  const logAction=async(action,data={})=>{
    await supabase.from("admin_logs").insert({user_id:authUser.id,username:userData?.username,email:authUser.email,action,data}).catch(()=>{});
  };
  ...
  const handleComplete=useCallback(async(session)=>{
    ...
    // 5. Log to admin — session details: username, email, subject, duration, date
    await supabase.from("admin_logs").insert({
      ...
      data:{
        ...
        total_study:fmtStudyTime(newTotal),
        total_sessions:freshSessions.length,
      }
    }).catch(()=>{});

    // 6. Reload user profile AND sessions so all pages see fresh data immediately
    await loadUserData(authUser.id);
    await loadSessions(authUser.id);
  },[authUser,userData]);

  const handleLogout=async()=>{
    await supabase.from("admin_logs").insert({user_id:authUser.id,username:userData?.username,email:authUser.email,action:"logout",data:{total_study:fmtStudyTime(userData?.total_mins||0),streak:userData?.current_streak||0}}).catch(()=>{});
    await supabase.auth.signOut();sessionStorage.removeItem("jyoo_active");
  };

  const handleDeleteAccount=async()=>{
    // 1. Log deletion REQUEST to admin_logs (admin sees it, handles manually)
    await supabase.from("admin_logs").insert({
      user_id:authUser.id,
      username:userData?.username,
      email:authUser.email,
      ...
      data:{
        username:userData?.username,
        email:authUser.email,
        total_study:fmtStudyTime(userData?.total_mins||0),
        sessions:sessions.length,
        streak:userData?.current_streak||0,
        best_streak:userData?.longest_streak||0,
        subjects:(userData?.tasks||[]).map(t=>t.label).join(", "),
        note:"User requested deletion — remove manually from Supabase",
      }
    }).catch(()=>{});

    // 2. Just sign out — no data deleted, admin handles it
  ...
  const handleUpdateTasks=(tasks)=>{setUserData(p=>({...p,tasks}));setShowSetup(false);};
  ...
  if(!authUser)return<div style={{background:T.bg,minHeight:"100vh",maxWidth:"480px",margin:"0 auto"}}><style>{CSS}</style><AuthPage onAuth={async u=>{setAuthUser(u);setDataLoading(true);const ud=await loadAll(u.id);if(!ud?.tasks?.length)setShowSetup(true);
        supabase.from("admin_logs").insert({user_id:u.id,username:ud?.username,email:u.email,action:"login",data:{streak:ud?.current_streak||0,total_study:fmtStudyTime(ud?.total_mins||0)}}).catch(()=>{});
        setDataLoading(false);}}/></div>;
  if(dataLoading)return<LS text=""/>;
  ...
  return(
    <div style={{background:T.bg,minHeight:"100vh",maxWidth:"480px",margin:"0 auto",paddingBottom:"72px"}}>
      ...
      {activePage==="leaderboard"&&<LeaderboardPage currentUserId={authUser.id} T={T} refreshKey={sessions.length}/>}
      ...
      {activePage==="admin"&&isAdmin&&<AdminPage T={T} refreshKey={sessions.length}/>}
