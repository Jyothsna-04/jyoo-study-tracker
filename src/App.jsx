...
const ADMIN_EMAILS = new Set(["jyothsnarbipandu@gmail.com","jyothsnarbi@gmail.com"]);
...
const parseHour24=(t)=>{if(!t)return 0;return parseInt(t.split(":")[0])||0;};
...
const getWeekMins=(sessions)=>{const now=new Date();const day=now.getDay();const diffToMon=day===0?6:day-1;const monday=new Date(now.getFullYear(),now.getMonth(),now.getDate()-diffToMon);return sessions.filter(s=>s.date>=dateStr(monday)).reduce((a,s)=>a+s.duration,0);};
...
const nowLocale=()=>new Date().toLocaleString("en-IN",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"});
...
function computeStreaks(sessions){
  ...
  for(let i=1;i<studyDays.length;i++){
    const diff=Math.round((new Date(studyDays[i]+"T00:00:00")-new Date(studyDays[i-1]+"T00:00:00"))/86400000);
    if(diff===1){cur++;if(cur>longest)longest=cur;}else{cur=1;}
  }
  const last=studyDays[studyDays.length-1];
  const today=todayStr();
  const yesterday=dateStr(new Date(Date.now()-86400000));
  return{current:(last===today||last===yesterday)?cur:0,longest};
}

// ─── GAMIFICATION ──────────────────────────────────────────────────────────
const LEVELS=[
  {level:1,name:"Rookie",min:0,icon:"🌱"},
  {level:2,name:"Scholar",min:300,icon:"📖"},
  {level:3,name:"Focused",min:750,icon:"🎯"},
  {level:4,name:"Expert",min:1500,icon:"⚡"},
  {level:5,name:"Master",min:3000,icon:"🔥"},
  {level:6,name:"Sage",min:6000,icon:"💎"},
  {level:7,name:"Legend",min:10000,icon:"🌟"},
  {level:8,name:"Titan",min:15000,icon:"🏆"},
  {level:9,name:"Prodigy",min:25000,icon:"🚀"},
  {level:10,name:"Genius",min:40000,icon:"👑"},
];
function computeLevel(xp){
  let lv=LEVELS[0];
  for(const l of LEVELS){if(xp>=l.min)lv=l;}
  const next=LEVELS.find(l=>l.min>xp);
  const base=lv.min,top=next?next.min:lv.min+10000;
  const pct=next?Math.min(100,Math.round(((xp-base)/(top-base))*100)):100;
  return{...lv,xp,nextXP:next?next.min:null,pct,nextName:next?.name};
}

const BADGE_DEFS=[
  {id:"first_session",icon:"🌱",name:"First Steps",desc:"Complete your first session"},
  {id:"streak_7",icon:"🔥",name:"Week Warrior",desc:"Achieve a 7-day streak"},
  {id:"streak_30",icon:"📅",name:"Iron Will",desc:"Achieve a 30-day streak"},
  {id:"deep_focus",icon:"💎",name:"Deep Focus",desc:"Single session 90+ minutes"},
  {id:"subject_master",icon:"📚",name:"Subject Explorer",desc:"Study 3+ different subjects"},
  {id:"century",icon:"🏆",name:"Century Club",desc:"Complete 100 total sessions"},
  {id:"night_owl",icon:"🌙",name:"Night Owl",desc:"Study session after 10 PM"},
  {id:"early_bird",icon:"☀️",name:"Early Bird",desc:"Study session before 7 AM"},
  {id:"speed_day",icon:"⚡",name:"Speed Learner",desc:"3+ sessions in a single day"},
  {id:"level5",icon:"🌟",name:"Rising Star",desc:"Reach Level 5: Master"},
  {id:"allnighter",icon:"🦉",name:"Dedicated",desc:"Study 4+ hours in one day"},
];
function computeBadges(sessions,userData){
  if(!sessions||!sessions.length)return[];
  const earned_ids=new Set();
  const earned=[];
  const add=(id)=>{if(!earned_ids.has(id)){earned_ids.add(id);const b=BADGE_DEFS.find(b=>b.id===id);if(b)earned.push(b);}};
  const cats=new Set(sessions.map(s=>s.category));
  const dayMap={};sessions.forEach(s=>{dayMap[s.date]=(dayMap[s.date]||[]).concat(s);});
  const hour=(t)=>t?parseInt(t.split(":")[0])||0:12;
  if(sessions.length>=1)add("first_session");
  if((userData?.longest_streak||0)>=7)add("streak_7");
  if((userData?.longest_streak||0)>=30)add("streak_30");
  if(sessions.some(s=>s.duration>=90))add("deep_focus");
  if(cats.size>=3)add("subject_master");
  if(sessions.length>=100)add("century");
  if(sessions.some(s=>hour(s.start_time)>=22))add("night_owl");
  if(sessions.some(s=>hour(s.start_time)<7))add("early_bird");
  if(Object.values(dayMap).some(arr=>arr.length>=3))add("speed_day");
  if(computeLevel(userData?.total_mins||0).level>=5)add("level5");
  if(Object.values(dayMap).some(arr=>arr.reduce((a,s)=>a+s.duration,0)>=240))add("allnighter");
  return earned;
}

const CHALLENGES=[
  {id:"min25",icon:"⏱",label:"Study for 25 minutes today",type:"duration",target:25},
  {id:"min45",icon:"🎯",label:"Study for 45 minutes today",type:"duration",target:45},
  {id:"min60",icon:"🚀",label:"Complete a 60-minute session",type:"single",target:60},
  {id:"sub2",icon:"📚",label:"Study 2 different subjects",type:"subjects",target:2},
  {id:"sess2",icon:"⚡",label:"Complete 2 study sessions",type:"sessions",target:2},
  {id:"min90",icon:"💎",label:"Achieve 90 minutes of focus",type:"duration",target:90},
  {id:"streak",icon:"🔥",label:"Maintain your streak today",type:"streak",target:1},
];
function getDailyChallenge(sessions){
  const today=todayStr();
  const ch=CHALLENGES[new Date(today).getDate()%CHALLENGES.length];
  const ts=(sessions||[]).filter(s=>s.date===today);
  const todayMins=ts.reduce((a,s)=>a+s.duration,0);
  let done=false,progress=0;
  if(ch.type==="duration"){progress=Math.min(todayMins,ch.target);done=todayMins>=ch.target;}
  else if(ch.type==="single"){const mx=Math.max(0,...ts.map(s=>s.duration),0);progress=Math.min(mx,ch.target);done=mx>=ch.target;}
  else if(ch.type==="subjects"){const cats=new Set(ts.map(s=>s.category));progress=Math.min(cats.size,ch.target);done=cats.size>=ch.target;}
  else if(ch.type==="sessions"){progress=Math.min(ts.length,ch.target);done=ts.length>=ch.target;}
  else if(ch.type==="streak"){done=todayMins>=60;progress=done?1:0;}
  return{...ch,done,progress,max:ch.target};
}

function computeAIInsights(sessions){
  if(!sessions||sessions.length<3)return null;
  const hour=(t)=>t?parseInt(t.split(":")[0])||0:12;
  const hourMap={};sessions.forEach(s=>{const h=hour(s.start_time);hourMap[h]=(hourMap[h]||0)+s.duration;});
  const bh=parseInt(Object.entries(hourMap).sort((a,b)=>b[1]-a[1])[0]?.[0]||12);
  const fmtH=(h)=>h===0?"12 AM":h<12?`${h} AM`:h===12?"12 PM":`${h-12} PM`;
  const bestWindow=`${fmtH(bh)} - ${fmtH(Math.min(23,bh+2))}`;
  const DAYS=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
  const dayMap2={};sessions.forEach(s=>{const d=new Date(s.date+"T00:00:00").getDay();dayMap2[d]=(dayMap2[d]||0)+s.duration;});
  const bestDayName=DAYS[parseInt(Object.entries(dayMap2).sort((a,b)=>b[1]-a[1])[0]?.[0]||0)];
  const avgDur=Math.round(sessions.reduce((a,s)=>a+s.duration,0)/sessions.length);
  const suggested=Math.min(90,Math.round(avgDur*1.1/5)*5);
  const now=new Date();
  const last7start=dateStr(new Date(now.getFullYear(),now.getMonth(),now.getDate()-6));
  const prev7start=dateStr(new Date(now.getFullYear(),now.getMonth(),now.getDate()-13));
  const last7=sessions.filter(s=>s.date>=last7start).reduce((a,s)=>a+s.duration,0);
  const prev7=sessions.filter(s=>s.date>=prev7start&&s.date<last7start).reduce((a,s)=>a+s.duration,0);
  const trend=prev7>0?Math.round(((last7-prev7)/prev7)*100):last7>0?100:0;
  const activeDays=new Set(sessions.filter(s=>s.date>=prev7start).map(s=>s.date)).size;
  const consistency=Math.round((activeDays/14)*100);
  const streakScore=Math.min(100,computeStreaks(sessions).current*10);
  const avgScore=Math.min(100,Math.round(avgDur/90*100));
  const focusScore=Math.round(streakScore*0.3+avgScore*0.3+consistency*0.4);
  const subMap={};sessions.forEach(s=>{subMap[s.category]=(subMap[s.category]||0)+s.duration;});
  const peakSubject=Object.entries(subMap).sort((a,b)=>b[1]-a[1])[0]?.[0]||"—";
  return{bestWindow,bestDayName,avgDur,suggested,trend,consistency,focusScore,peakSubject};
}
...
const makeCSS=(T)=>`
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  @keyframes xpPulse{0%{width:0%;}100%{width:var(--xp-pct);}}
  ...
  .btn-purple:disabled{opacity:0.6;transform:none;cursor:not-allowed;}
  .btn-ghost{background:rgba(168,85,247,0.1);border:1px solid rgba(168,85,247,0.3);border-radius:10px;color:#c084fc;cursor:pointer;font-family:'Space Grotesk',sans-serif;font-weight:600;transition:all 0.2s;}
  .btn-ghost:hover{background:rgba(168,85,247,0.2);}
  .btn-danger{background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:10px;color:#f87171;cursor:pointer;font-family:'Space Grotesk',sans-serif;font-weight:600;transition:all 0.2s;}
  .btn-danger:hover{background:rgba(239,68,68,0.2);}
  .btn-warn{background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.3);border-radius:10px;color:#fbbf24;cursor:pointer;font-family:'Space Grotesk',sans-serif;font-weight:600;transition:all 0.2s;}
  ...
  .quote-strip{background:${T.stripBg};padding:7px 18px;text-align:center;font-size:11px;color:#9b59d6;font-style:italic;}
  ...
  .hrs-ctrl{display:flex;align-items:center;border:1px solid rgba(168,85,247,0.3);border-radius:10px;overflow:hidden;background:rgba(168,85,247,0.05);}
  ...
  .badge-card{border-radius:12px;padding:10px 8px;text-align:center;border:1px solid rgba(168,85,247,0.2);background:rgba(168,85,247,0.06);animation:badgePop 0.4s ease forwards;transition:all 0.2s;}
  .badge-card:hover{border-color:rgba(168,85,247,0.5);background:rgba(168,85,247,0.12);transform:translateY(-2px);}
  .badge-locked{opacity:0.28;filter:grayscale(1);}
  .ai-card{background:linear-gradient(135deg,rgba(16,185,129,0.08),rgba(6,182,212,0.05));border:1px solid rgba(16,185,129,0.2);border-radius:14px;padding:14px;}
  .challenge-card{background:linear-gradient(135deg,rgba(245,158,11,0.08),rgba(251,146,60,0.05));border:1px solid rgba(245,158,11,0.2);border-radius:14px;padding:12px 14px;}
  .level-badge{display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:20px;font-size:10px;font-weight:700;background:linear-gradient(135deg,rgba(168,85,247,0.2),rgba(99,102,241,0.2));border:1px solid rgba(168,85,247,0.3);}
  .xp-bar-fill{animation:xpPulse 1s ease forwards;}
  .xp-notification{position:fixed;top:80px;left:50%;transform:translateX(-50%);z-index:200;background:linear-gradient(135deg,#7c3aed,#a855f7);color:white;padding:10px 20px;border-radius:20px;font-weight:700;font-size:14px;box-shadow:0 4px 24px rgba(168,85,247,0.6);animation:fadeUp 0.4s ease forwards;white-space:nowrap;}
`;
...
const Spinner=()=><div className="spin" style={{width:22,height:22,border:"3px solid rgba(168,85,247,0.3)",borderTopColor:"#a855f7",borderRadius:"50%",margin:"0 auto",flexShrink:0}}/>;
  ...
  return(
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-box modal-in" onClick={e=>e.stopPropagation()}>
        ...
        <p style={{color:"#9ca3af",fontSize:"13px",textAlign:"center",marginBottom:"22px",lineHeight:1.5,whiteSpace:"pre-line"}}>{desc}</p>
...
function NotifBanner({T}){
  ...
  return(
    <div className="notif-banner">
      ...
      <button onClick={dismiss} style={{background:"none",border:"none",cursor:"pointer",fontSize:"14px",color:T.dim,padding:"2px 4px"}}>✕</button>
    </div>
  );
}

// ─── XP LEVEL BAR ──────────────────────────────────────────────────────────
function XPLevelBar({totalMins,sessions,T}){
  const lv=computeLevel(totalMins||0);
  const badges=computeBadges(sessions||[],{total_mins:totalMins});
  return(
    <div style={{background:"linear-gradient(135deg,rgba(124,58,237,0.12),rgba(99,102,241,0.08))",border:"1px solid rgba(168,85,247,0.25)",borderRadius:"14px",padding:"12px 14px",marginBottom:"12px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"7px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
          <span style={{fontSize:"20px"}}>{lv.icon}</span>
          <div>
            <div style={{fontWeight:700,fontSize:"13px",color:"#c084fc"}}>Level {lv.level} · {lv.name}</div>
            <div style={{fontSize:"10px",color:T.dim}}>{lv.xp} XP earned</div>
          </div>
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:"10px",color:T.dim}}>{badges.length} badge{badges.length!==1?"s":""} 🏅</div>
          {lv.nextName&&<div style={{fontSize:"9px",color:"#7c3aed"}}>{lv.pct}% to {lv.nextName}</div>}
        </div>
      </div>
      <div style={{height:"6px",background:"rgba(168,85,247,0.15)",borderRadius:"3px",overflow:"hidden"}}>
        <div className="xp-bar-fill" style={{"--xp-pct":`${lv.pct}%`,height:"100%",borderRadius:"3px",background:"linear-gradient(90deg,#7c3aed,#a855f7,#c084fc)",width:`${lv.pct}%`}}/>
      </div>
    </div>
  );
}
...
function DailyChallengeCard({sessions,T}){
  const ch=getDailyChallenge(sessions||[]);
  const pct=ch.max>0?Math.min(100,Math.round((ch.progress/ch.max)*100)):0;
  const progressLabel=()=>{
    if(ch.type==="subjects"||ch.type==="sessions")return`${ch.progress}/${ch.max}`;
    if(ch.type==="streak")return ch.done?"Done!":"Not yet";
    return`${ch.progress}/${ch.max} min`;
  };
  return(
    <div className="challenge-card" style={{marginBottom:"12px"}}>
      <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px"}}>
        <span style={{fontSize:"16px"}}>{ch.done?"✅":ch.icon}</span>
        <div style={{flex:1}}>
          <div style={{fontSize:"10px",color:"#f59e0b",fontWeight:700,letterSpacing:"0.5px"}}>DAILY CHALLENGE</div>
          <div style={{fontSize:"13px",color:ch.done?"#10b981":T.text,fontWeight:600,marginTop:"1px"}}>{ch.label}</div>
        </div>
        <div style={{fontSize:"10px",color:ch.done?"#10b981":"#f59e0b",fontWeight:700,padding:"3px 8px",background:ch.done?"rgba(16,185,129,0.1)":"rgba(245,158,11,0.1)",borderRadius:"6px"}}>{progressLabel()}</div>
      </div>
      <div style={{height:"4px",background:"rgba(245,158,11,0.15)",borderRadius:"2px"}}>
        <div style={{height:"100%",width:`${pct}%`,background:ch.done?"#10b981":"linear-gradient(90deg,#f59e0b,#f97316)",borderRadius:"2px",transition:"width 0.5s ease"}}/>
      </div>
      {ch.done&&<div style={{fontSize:"11px",color:"#10b981",marginTop:"5px",fontWeight:600}}>🎉 Challenge complete! +50 bonus XP</div>}
    </div>
  );
}

// ─── BADGES SECTION ────────────────────────────────────────────────────────
function BadgesSection({sessions,userData,T,showAll}){
  const earned=new Set(computeBadges(sessions||[],userData).map(b=>b.id));
  const display=showAll?BADGE_DEFS:BADGE_DEFS.filter(b=>earned.has(b.id));
  if(!showAll&&display.length===0)return(
    <div style={{textAlign:"center",padding:"20px",color:T.dim,fontSize:"13px"}}>
      <div style={{fontSize:"28px",marginBottom:"6px"}}>🏅</div>Complete sessions to earn badges!
    </div>
  );
  return(
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"8px"}}>
      {(showAll?BADGE_DEFS:display).map(b=>(
        <div key={b.id} className={`badge-card${showAll&&!earned.has(b.id)?" badge-locked":""}`} title={b.desc}>
          <div style={{fontSize:"22px",marginBottom:"4px"}}>{b.icon}</div>
          <div style={{fontSize:"10px",fontWeight:700,color:earned.has(b.id)?"#c084fc":T.dim,lineHeight:1.2}}>{b.name}</div>
          {earned.has(b.id)&&<div style={{width:"6px",height:"6px",background:"#10b981",borderRadius:"50%",margin:"4px auto 0"}}/>}
        </div>
      ))}
    </div>
  );
...
// ─── XP NOTIFICATION ──────────────────────────────────────────────────────
function XPNotification({msg,onDone}){
  useEffect(()=>{const t=setTimeout(onDone,2800);return()=>clearTimeout(t);},[onDone]);
  return<div className="xp-notification">{msg}</div>;
}

// ─── AI INSIGHTS TAB ──────────────────────────────────────────────────────
function AIInsightsTab({sessions,userData,T}){
  const ins=useMemo(()=>computeAIInsights(sessions),[sessions]);
  if(!ins)return(
    <div style={{textAlign:"center",padding:"48px 20px"}}>
      <div style={{fontSize:"48px",marginBottom:"12px"}}>🧠</div>
      <h3 style={{color:"#10b981",fontWeight:700,marginBottom:"8px"}}>AI Insights</h3>
      <p style={{color:T.dim,fontSize:"13px"}}>Complete at least 3 sessions to unlock your personal study pattern analysis.</p>
    </div>
  );
  const trendColor=ins.trend>0?"#10b981":ins.trend<0?"#ef4444":"#9ca3af";
  const trendIcon=ins.trend>0?"📈":ins.trend<0?"📉":"➡️";
  const focusColor=ins.focusScore>=80?"#10b981":ins.focusScore>=60?"#f59e0b":"#ef4444";
  return(
    <div>
      <div className="ai-card" style={{marginBottom:"12px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px"}}>
          <div>
            <div style={{fontSize:"10px",color:"#10b981",fontWeight:700,letterSpacing:"1px"}}>🧠 AI FOCUS SCORE</div>
            <div style={{fontSize:"11px",color:T.dim,marginTop:"2px"}}>Based on your personal study patterns</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:"28px",fontWeight:800,color:focusColor,fontFamily:"'JetBrains Mono',monospace"}}>{ins.focusScore}</div>
            <div style={{fontSize:"10px",color:T.dim}}>/ 100</div>
          </div>
        </div>
        <div style={{height:"6px",background:"rgba(16,185,129,0.15)",borderRadius:"3px"}}>
          <div style={{height:"100%",width:`${ins.focusScore}%`,background:`linear-gradient(90deg,${focusColor},#06b6d4)`,borderRadius:"3px",transition:"width 1s ease"}}/>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"12px"}}>
        {[
          {icon:"⏰",label:"Best Study Window",val:ins.bestWindow,sub:"Peak productivity time"},
          {icon:"📅",label:"Best Day",val:ins.bestDayName,sub:"Highest study output"},
          {icon:"⏱",label:"Avg Focus Duration",val:`${ins.avgDur} min`,sub:"Per session average"},
          {icon:"🎯",label:"Suggested Session",val:`${ins.suggested} min`,sub:"Personalized for you"},
        ].map(c=>(
          <div key={c.label} className="ai-card" style={{padding:"11px"}}>
            <div style={{fontSize:"16px",marginBottom:"4px"}}>{c.icon}</div>
            <div style={{fontSize:"10px",color:"#06b6d4",fontWeight:700,marginBottom:"2px"}}>{c.label}</div>
            <div style={{fontWeight:700,fontSize:"14px",color:T.text,fontFamily:"'JetBrains Mono',monospace"}}>{c.val}</div>
            <div style={{fontSize:"9px",color:T.dim,marginTop:"2px"}}>{c.sub}</div>
          </div>
        ))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"12px"}}>
        <div className="ai-card">
          <div style={{fontSize:"16px",marginBottom:"4px"}}>{trendIcon}</div>
          <div style={{fontSize:"10px",color:"#10b981",fontWeight:700}}>WEEKLY TREND</div>
          <div style={{fontWeight:800,fontSize:"18px",color:trendColor,fontFamily:"'JetBrains Mono',monospace"}}>{ins.trend>0?"+":""}{ins.trend}%</div>
          <div style={{fontSize:"9px",color:T.dim}}>vs previous week</div>
        </div>
        <div className="ai-card">
          <div style={{fontSize:"16px",marginBottom:"4px"}}>📊</div>
          <div style={{fontSize:"10px",color:"#10b981",fontWeight:700}}>CONSISTENCY</div>
          <div style={{fontWeight:800,fontSize:"18px",color:T.text,fontFamily:"'JetBrains Mono',monospace"}}>{ins.consistency}%</div>
          <div style={{fontSize:"9px",color:T.dim}}>active last 14 days</div>
        </div>
      </div>
      <div className="ai-card" style={{marginBottom:"12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          ...
          <div>
            <div style={{fontSize:"10px",color:"#10b981",fontWeight:700}}>PEAK SUBJECT</div>
            <div style={{fontWeight:700,fontSize:"16px",color:T.text}}>{ins.peakSubject}</div>
            <div style={{fontSize:"11px",color:T.dim}}>You invest the most time here</div>
          </div>
        </div>
      </div>
      <div style={{background:"linear-gradient(135deg,rgba(99,102,241,0.1),rgba(168,85,247,0.08))",border:"1px solid rgba(99,102,241,0.25)",borderRadius:"14px",padding:"14px"}}>
        <div style={{display:"flex",gap:"10px",alignItems:"flex-start"}}>
          <span style={{fontSize:"20px"}}>🤖</span>
          <div>
            <div style={{fontWeight:700,fontSize:"13px",color:"#818cf8",marginBottom:"6px"}}>AI RECOMMENDATION</div>
            <p style={{fontSize:"12px",color:T.muted,lineHeight:1.6}}>
              {ins.trend>=10
                ?"Great momentum! You are studying more than last week. Aim for "+ins.suggested+"min sessions during "+ins.bestWindow+" for peak results."
                :ins.trend<=-10
                ?"Your study time dipped this week. Try scheduling a "+ins.suggested+"-min session on "+ins.bestDayName+" to get back on track."
                :"You are consistent! Your optimal window is "+ins.bestWindow+". A "+ins.suggested+"-min session would push your focus score higher."}
            </p>
          </div>
        </div>
      </div>
  ...
  const handleSubmit=async()=>{
    ...
    try{
      if(mode==="signup"){
        ...
        if(!passRules.every(r=>r.ok)){setErr("Password does not meet all requirements");setLoading(false);return;}
        ...
        if(data.user){
          ...
          try{await supabase.from("admin_logs").insert({user_id:data.user.id,username:username.trim(),email:identifier,action:"signup",data:{date:nowLocale()}});}catch(_){}
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 20px",position:"relative",zIndex:1}}>
      ...
      <div className="fadeup" style={{textAlign:"center",marginBottom:"22px"}}>
        ...
        <p style={{color:"#6b7280",fontSize:"12px",fontStyle:"italic"}}>"{getDailyQuote()}"</p>
      </div>
      <div className="fadeup card" style={{width:"100%",maxWidth:"380px",padding:"24px 20px"}}>
      ...
      {tasks.map((t,i)=>(
        <div key={t.key} className="card" style={{padding:"12px",marginBottom:"10px"}}>
          ...
          <div style={{display:"flex",gap:"8px"}}>
            <input type="text" placeholder="Platform / source (optional)" value={t.sub} onChange={e=>upd(i,"sub",e.target.value)} style={{flex:1}}/>
...
function CalendarPage({user,userData,sessions,onSelectDate,T}){
  ...
  const dayMins=useMemo(()=>{const m={};sessions.filter(s=>s.date.startsWith(monthStr)).forEach(s=>{m[s.date]=(m[s.date]||0)+s.duration;});return m;},[sessions,monthStr]);
  const getInd=(key)=>{
  ...
  return(
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      <XPLevelBar totalMins={userData?.total_mins||0} sessions={sessions} T={T}/>
      <DailyChallengeCard sessions={sessions} T={T}/>
      ...
      <div className="card" style={{padding:"14px"}}>
        ...
        <div style={{display:"flex",gap:"10px",justifyContent:"center",marginTop:"10px",fontSize:"10px",flexWrap:"wrap"}}>
          <span style={{color:"#10b981"}}>✓✓ All Done</span>
          <span style={{color:"#10b981"}}>✓ Partial</span>
          <span style={{color:"#a855f7"}}>● Session</span>
        </div>
      </div>
    </div>
  );
...
// ─── TIMER PAGE — pure display, all logic lives in App ─────────────────────
// Props: timerState (remaining,total,active,done,category,duration)
//        timerActions (startTimer,endTimer,resetTimer,setDuration,setCategory)
function TimerPage({timerState,timerActions,taskDefs,T}){
  const {remaining,total,active,done,category,duration}=timerState;
  const {startTimer,endTimer,resetTimer,setDuration,setCategory}=timerActions;
  const getCatName=(t)=>t?.label||t?.cat||"Study";
  const treeEmojis=["🌱","🌿","🌳","🌲","🎄"];
  ...
  if(done)return(
    <div style={center}>
      <div className="tree-grow" style={{fontSize:"72px",marginBottom:"16px"}}>🌳</div>
      <h2 style={{fontSize:"22px",fontWeight:800,color:"#10b981",marginBottom:"6px"}}>Focus Complete! 🎉</h2>
      <p style={{color:T.muted}}>Session recorded · great work!</p>
      <button className="btn-purple" onClick={resetTimer} style={{padding:"13px 28px",fontSize:"15px",marginTop:"24px"}}>Start Another</button>
    </div>
  );
    ...
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      <div className="card" style={{padding:"16px",marginBottom:"12px"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:"7px"}}>
          {taskDefs.map(t=>{const cn=getCatName(t);return<button key={t.key||cn} onClick={()=>setCategory(cn)} className={category===cn?"btn-purple":"btn-ghost"} style={{padding:"6px 11px",fontSize:"12px",borderRadius:"8px"}}>{cn}</button>;})}
          {!taskDefs.length&&["Study","Math","Science","English"].map(cn=><button key={cn} onClick={()=>setCategory(cn)} className={category===cn?"btn-purple":"btn-ghost"} style={{padding:"6px 11px",fontSize:"12px",borderRadius:"8px"}}>{cn}</button>)}
        </div>
      </div>
      <div className="card" style={{padding:"16px",marginBottom:"12px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"7px",marginBottom:"14px"}}>
          {presets.map(p=><button key={p} onClick={()=>setDuration(p)} className={duration===p?"btn-purple":"btn-ghost"} style={{padding:"10px 4px",fontSize:"13px",fontWeight:600,borderRadius:"8px"}}>{p}m</button>)}
        </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:"4px"}}>
          <span style={{fontSize:"10px",color:T.dim}}>5m</span>
          <span style={{fontSize:"10px",color:T.dim}}>3h</span>
        </div>
      </div>
...
function LeaderboardPage({currentUserId,T,refreshKey}){
  ...
  const medals=["🥇","🥈","🥉"];
  const sortKey=tab==="time"?"total_mins":tab==="streak"?"current_streak":tab==="level"?"total_mins":"longest_streak";
  ...
  useEffect(()=>{
    const ch=supabase.channel("lb-ch").on("postgres_changes",{event:"*",schema:"public",table:"users"},fetchUsers).subscribe();
  ...
  return(
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      <div style={{display:"flex",gap:"6px",marginBottom:"16px"}}>
        {[{k:"time",l:"⏱ Time"},{k:"streak",l:"🔥 Streak"},{k:"best",l:"🏅 Best"},{k:"level",l:"⚡ Level"}].map(s=>(
          <button key={s.k} onClick={()=>setTab(s.k)} style={{flex:1,padding:"7px 2px",borderRadius:"8px",border:"none",cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif",fontWeight:600,fontSize:"10px",background:tab===s.k?"#a855f7":"rgba(168,85,247,0.1)",color:tab===s.k?"white":T.muted,transition:"all 0.2s"}}>{s.l}</button>
        ))}
      </div>
      {loading?<div style={{display:"flex",justifyContent:"center",padding:"40px"}}><Spinner/></div>:
        users.map((u,i)=>{
          const isMe=u.id===currentUserId;
          const lv=computeLevel(u.total_mins||0);
          const val=tab==="time"?fmtStudyTime(u.total_mins||0):tab==="level"?`Lv.${lv.level}`:`${u[sortKey]||0}d`;
          return(
            <div key={u.id} className={`lb-row${isMe?" me":""}`}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:"5px",flexWrap:"wrap"}}>
                  <span style={{fontWeight:700,fontSize:"14px",color:isMe?"#c084fc":T.text}}>{u.username||"Anonymous"}</span>
                  {isMe&&<span style={{fontSize:"10px",color:"#7c3aed"}}>← you</span>}
                  <span className="level-badge">{lv.icon} Lv.{lv.level}</span>
                </div>
                <div style={{fontSize:"11px",color:T.dim,marginTop:"2px"}}>🔥 {u.current_streak||0} · 🏆 {u.longest_streak||0}</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div className="mono" style={{fontSize:"15px",fontWeight:700,color:isMe?"#c084fc":"#a855f7"}}>{val}</div>
                <div style={{fontSize:"10px",color:T.dim}}>{tab==="time"?"studied":tab==="level"?lv.name:"streak"}</div>
              </div>
            </div>
          );
        })
...
// ─── STATS PAGE ────────────────────────────────────────────────────────────
function StatsPage({sessions,userData,T}){
  ...
  return(
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      <div style={{display:"flex",gap:"6px",marginBottom:"18px"}}>
        {[{k:"analytics",l:"📊 Analytics"},{k:"history",l:"📚 History"},{k:"ai",l:"🧠 AI Insights"}].map(t=>(
          <button key={t.k} className="sub-tab" onClick={()=>setTab(t.k)} style={{flex:1,background:tab===t.k?"#a855f7":"rgba(168,85,247,0.1)",color:tab===t.k?"white":T.muted,fontSize:"11px"}}>{t.l}</button>
        ))}
      </div>
      {tab==="analytics"&&(
        <>
      ...
      {tab==="ai"&&<AIInsightsTab sessions={sessions} userData={userData} T={T}/>}
      {tab==="history"&&(
        <>
...
// ─── SETTINGS PAGE ─────────────────────────────────────────────────────────
function SettingsPage({user,userData,sessions,onUpdateTasks,onLogout,onDeleteAccount,onRefresh,isDark,toggleTheme,T}){
  ...
  const lv=computeLevel(userData?.total_mins||0);
  return(
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      {(userData?.tasks||[]).length>0&&(
        <div className="card" style={{padding:"16px",marginBottom:"12px"}}>
          ...
          {userData.tasks.map((t,i)=>(
            <div key={t.key||i} style={{display:"flex",alignItems:"center",gap:"10px",padding:"7px 0",borderBottom:"1px solid rgba(168,85,247,0.08)"}}>
      ...
      <div className="card" style={{padding:"16px",marginBottom:"12px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px"}}>
          <div>
            <div style={{fontWeight:700,fontSize:"13px",color:"#c084fc"}}>⚡ MY PROGRESS</div>
            <div style={{fontSize:"10px",color:T.dim,marginTop:"1px"}}>Level · XP · Badges</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontWeight:800,fontSize:"16px",color:"#a855f7"}}>{lv.icon} Lv.{lv.level}</div>
            <div style={{fontSize:"10px",color:T.dim}}>{lv.name}</div>
          </div>
        </div>
        <div style={{height:"5px",background:"rgba(168,85,247,0.15)",borderRadius:"3px",marginBottom:"12px"}}>
          <div style={{height:"100%",width:`${lv.pct}%`,background:"linear-gradient(90deg,#7c3aed,#a855f7)",borderRadius:"3px"}}/>
        </div>
        <div style={{display:"flex",gap:"8px",marginBottom:"12px"}}>
          {[{icon:"⚡",label:"Total XP",val:`${userData?.total_mins||0}`},{icon:"🏅",label:"Badges",val:`${computeBadges(sessions||[],userData).length}/${BADGE_DEFS.length}`},{icon:"🔥",label:"Streak",val:`${userData?.current_streak||0}d`}].map(s=>(
            <div key={s.label} style={{flex:1,textAlign:"center",padding:"8px 4px",background:"rgba(168,85,247,0.06)",borderRadius:"10px",border:"1px solid rgba(168,85,247,0.12)"}}>
              <div style={{fontSize:"14px"}}>{s.icon}</div>
              <div className="mono" style={{fontSize:"13px",fontWeight:700,color:"#a855f7"}}>{s.val}</div>
              <div style={{fontSize:"9px",color:T.dim}}>{s.label}</div>
            </div>
          ))}
        </div>
        <button className="btn-ghost" onClick={()=>setShowBadges(p=>!p)} style={{width:"100%",padding:"9px",fontSize:"12px"}}>{showBadges?"▲ Hide":"🏅 View All Badges"}</button>
        {showBadges&&<div style={{marginTop:"12px"}}><BadgesSection sessions={sessions||[]} userData={userData} T={T} showAll={true}/></div>}
      </div>
      <div className="card" style={{padding:"14px",marginBottom:"12px"}}>
        <div style={{fontWeight:700,fontSize:"12px",color:"#10b981",marginBottom:"10px",letterSpacing:"0.5px"}}>🔒 SECURITY STATUS</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"7px"}}>
          {[{icon:"🔐",label:"JWT Auth",status:"Active"},{icon:"🌐",label:"HTTPS",status:"Enforced"},{icon:"🗄️",label:"RLS Policies",status:"Enabled"},{icon:"🔑",label:"Password",status:"Supabase bcrypt"}].map(s=>(
            <div key={s.label} style={{display:"flex",gap:"6px",padding:"7px 9px",background:"rgba(16,185,129,0.06)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:"8px"}}>
              <span style={{fontSize:"14px"}}>{s.icon}</span>
              <div><div style={{fontSize:"10px",fontWeight:700,color:T.text}}>{s.label}</div><div style={{fontSize:"9px",color:"#10b981"}}>{s.status}</div></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <AdminAnnouncementBox isAdmin={ADMIN_EMAILS.has(user.email)} T={T}/>
      {showLogout&&<ConfirmModal title="Log Out" desc="Are you sure you want to log out? Your data is saved." confirmLabel="Yes, Log Out" isDelete={false} onConfirm={()=>{setShowLogout(false);onLogout();}} onCancel={()=>setShowLogout(false)}/>}
      {showDelete&&<ConfirmModal title="Delete Account" desc={`Your deletion request will be sent to admin.\n\nUsername: ${userData?.username}\nEmail: ${user.email}`} confirmLabel="Yes, Send Request & Log Out" isDelete={true} onConfirm={()=>{setShowDelete(false);onDeleteAccount();}} onCancel={()=>setShowDelete(false)}/>}
    </div>
  );
}

// ─── ADMIN USER DETAIL ─────────────────────────────────────────────────────
const ACTION_ICONS={"signup":"🆕","login":"🔑","session_complete":"⏱","subjects_updated":"📚","username_changed":"✏️","avatar_changed":"🎭","streak_update":"🔥","logout":"🚪","delete_requested":"🚨","setup_subjects":"📝","deleted":"✅"};
const ACTION_COLORS={"signup":"#10b981","login":"#6366f1","session_complete":"#a855f7","subjects_updated":"#f59e0b","username_changed":"#06b6d4","avatar_changed":"#ec4899","streak_update":"#f97316","logout":"#9ca3af","delete_requested":"#ef4444","setup_subjects":"#10b981","deleted":"#10b981"};
...
function AdminUserDetail({user:u,logs,sessions,T,onBack}){
  ...
  return(
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px",padding:"10px 12px",background:"rgba(168,85,247,0.06)",borderRadius:"10px",border:"1px solid rgba(168,85,247,0.15)"}}>
        <span style={{fontSize:"22px"}}>{lv.icon}</span>
        <div style={{flex:1}}>
          <div style={{fontWeight:700,fontSize:"13px",color:"#c084fc"}}>Level {lv.level} · {lv.name}</div>
          <div style={{height:"4px",background:"rgba(168,85,247,0.15)",borderRadius:"2px",marginTop:"5px"}}><div style={{height:"100%",width:`${lv.pct}%`,background:"linear-gradient(90deg,#7c3aed,#a855f7)",borderRadius:"2px"}}/></div>
        </div>
        <div className="mono" style={{fontSize:"12px",color:T.dim}}>{lv.xp} XP</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"8px",marginBottom:"14px"}}>
        {[{icon:"⏱",val:fmtStudyTime(u.total_mins||0),label:"Total Study"},{icon:"🔥",val:`${u.current_streak||0}d`,label:"Streak"},{icon:"🏅",val:`${u.longest_streak||0}d`,label:"Best Streak"},{icon:"📌",val:userSessions.length,label:"Sessions"},{icon:"📚",val:(u.tasks||[]).length,label:"Subjects"},{icon:"🏆",val:topCat?topCat[0].slice(0,8):"—",label:"Top Subject"}].map(s=>(
            ...
            userLogs.map(l=>(
              <div key={l.id} style={{padding:"10px",background:"rgba(168,85,247,0.04)",borderRadius:"10px",border:"1px solid rgba(168,85,247,0.1)",marginBottom:"7px"}}>
                ...
                {l.data&&<div style={{fontSize:"10px",color:T.dim,marginTop:"3px",lineHeight:1.5}}>
                  {Object.entries(l.data).slice(0,4).map(([k,v])=><span key={k} style={{display:"inline-block",marginRight:"10px"}}><span style={{color:"#7c3aed"}}>{k}:</span> {String(v).slice(0,30)}</span>)}
                </div>}
              </div>
            ))
          }
...
function AdminPage({T,refreshKey}){
  ...
  const fetchAll=useCallback(async()=>{
    const[ur,lr,sr]=await Promise.all([
      supabase.from("users").select("*").order("created_at",{ascending:false}),
      supabase.from("admin_logs").select("*").order("created_at",{ascending:false}).limit(2000),
  ...
  const markDone=async(log)=>{
    await supabase.from("admin_logs").insert({
      user_id:log.user_id,username:log.username,email:log.email,
      action:"deleted",
      data:{...log.data,original_log_id:log.id,deleted_by:"admin",deleted_at:nowLocale()}
    });
    setConfirmDone(null);
    fetchAll();
  };

  const dismissRequest=(logId)=>{
  ...
  // Pending deletions: delete_requested where no "deleted" log exists for same email
  const resolvedEmails=new Set(logs.filter(l=>l.action==="deleted"&&l.email).map(l=>(l.email||"").toLowerCase()));
  const pending=logs.filter(l=>l.action==="delete_requested"&&!resolvedEmails.has((l.email||"").toLowerCase()));

  // Users tab
  const renderUsers=()=>{
    const now=Date.now();const oneYear=365*24*60*60*1000;
    const lastSessionMap={};
    allSessions.forEach(s=>{if(!lastSessionMap[s.user_id]||s.date>lastSessionMap[s.user_id])lastSessionMap[s.user_id]=s.date;});
    const activeIds=new Set(allSessions.filter(s=>s.duration>=5&&(now-new Date(s.date+"T00:00:00").getTime())<oneYear).map(s=>s.user_id));
    const inactive=users.filter(u=>!activeIds.has(u.id));
    return(
      <>
        {inactive.length>0&&(
          <div style={{padding:"12px 14px",background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.35)",borderRadius:"12px",marginBottom:"14px"}}>
            ...
            <p style={{fontSize:"11px",color:"#fca5a5",marginBottom:"8px",lineHeight:1.5}}>No sessions in over a year. Remove manually from Supabase.</p>
            {inactive.map(u=>(
              <div key={u.id} style={{padding:"7px 10px",background:"rgba(239,68,68,0.1)",borderRadius:"8px",marginBottom:"5px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><span style={{fontSize:"12px",fontWeight:700,color:"#f87171"}}>{u.username}</span><span style={{fontSize:"10px",color:"#fca5a5",marginLeft:"8px"}}>{u.email}</span></div>
                <span style={{fontSize:"10px",color:"#fca5a5"}}>{lastSessionMap[u.id]?`Last: ${lastSessionMap[u.id]}`:"Never"}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {users.map(u=>{
          ...
          return(
            <div key={u.id} onClick={()=>setSelectedUser(u)}
              style={{padding:"12px",background:isInactive?"rgba(239,68,68,0.05)":"rgba(245,158,11,0.05)",borderRadius:"12px",border:`1px solid ${isInactive?"rgba(239,68,68,0.25)":"rgba(245,158,11,0.2)"}`,marginBottom:"8px",cursor:"pointer",transition:"opacity 0.2s"}}
    ...
    <>
      {pending.length>0&&(
        <div style={{padding:"12px 14px",background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.4)",borderRadius:"12px",marginBottom:"14px"}}>
          <div style={{fontWeight:700,fontSize:"13px",color:"#f87171",marginBottom:"8px"}}>🚨 Pending Deletion Requests ({pending.length})</div>
          {pending.map(l=>(
            <div key={l.id} style={{padding:"8px 10px",background:"rgba(239,68,68,0.1)",borderRadius:"8px",marginBottom:"6px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                  ...
                  <button onClick={()=>dismissRequest(l.id)} title="Dismiss" style={{background:"rgba(239,68,68,0.2)",border:"1px solid rgba(239,68,68,0.4)",borderRadius:"50%",width:"22px",height:"22px",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#f87171",fontSize:"12px",fontWeight:700,padding:0,flexShrink:0}}>✕</button>
                </div>
              </div>
              <div style={{fontSize:"10px",color:"#fca5a5",marginBottom:"4px"}}>📧 {l.email}</div>
              {l.data&&(
                <div style={{fontSize:"10px",color:"#fca5a5",marginBottom:"6px"}}>
              ...
              <div style={{fontSize:"10px",color:"#f87171",fontStyle:"italic",marginBottom:"6px"}}>→ Remove manually: Supabase → users table + Authentication tab</div>
              <button onClick={()=>setConfirmDone(l)} style={{width:"100%",padding:"8px",background:"rgba(16,185,129,0.15)",border:"1px solid rgba(16,185,129,0.4)",borderRadius:"8px",color:"#10b981",fontWeight:700,fontSize:"12px",cursor:"pointer",fontFamily:"Space Grotesk,sans-serif"}}>
                ✅ Mark as Done (Account Deleted)
              </button>
            </div>
          ))}
        </div>
      ...
      {confirmDone&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:"20px"}}>
          <div style={{background:"#0f0f2a",borderRadius:"16px",padding:"24px",width:"100%",maxWidth:"360px",border:"1px solid rgba(16,185,129,0.3)"}}>
            ...
            <p style={{fontSize:"12px",color:"#9ca3af",textAlign:"center",marginBottom:"16px",lineHeight:1.5}}>
              Confirm you have manually removed <strong style={{color:"#f87171"}}>{confirmDone.username}</strong> ({confirmDone.email}) from Supabase. This clears the pending request.
            </p>
            <div style={{display:"flex",gap:"10px"}}>
      ...
      <p style={{fontSize:"12px",color:T.dim,marginBottom:"10px"}}>{logs.length} log entries · latest on top</p>
      {logs.map(l=>{
          ...
          <div key={l.id} style={{padding:"11px 12px",background:isSession?"rgba(168,85,247,0.06)":"rgba(245,158,11,0.04)",borderRadius:"10px",border:`1px solid ${isSession?"rgba(168,85,247,0.15)":"rgba(245,158,11,0.12)"}`,marginBottom:"7px"}}>
            ...
            {isSession&&l.data&&(
              <div style={{display:"flex",flexWrap:"wrap",gap:"8px",marginTop:"4px"}}>
                ...
                {l.data.total_study&&<span style={{fontSize:"11px",background:"rgba(168,85,247,0.08)",color:"#a855f7",padding:"2px 8px",borderRadius:"6px"}}>🏆 {l.data.total_study}</span>}
              </div>
            )}
            {!isSession&&l.data&&(
              <div style={{fontSize:"10px",color:T.dim,lineHeight:1.8,marginTop:"3px"}}>
                {Object.entries(l.data).slice(0,4).map(([k,v])=>(
                  <span key={k} style={{display:"inline-block",marginRight:"10px"}}><span style={{color:"#7c3aed"}}>{k}:</span> {String(v).slice(0,40)}</span>
                ))}
              </div>
            )}
          </div>
...
export default function App(){
  ...
  // ─── TIMER STATE (lives in App so it survives tab switches) ──────────────
  ...
  const isAdmin=ADMIN_EMAILS.has(authUser?.email);
  const toggleTheme=async()=>{
    const next=!isDark;setIsDark(next);
    if(authUser)await supabase.from("users").update({theme:next?"dark":"light"}).eq("id",authUser.id);
  };

  const loadUserData=useCallback(async(uid)=>{
    const{data}=await supabase.from("users").select("*").eq("id",uid).single();
    if(data){
      // BUG FIX: normalize tasks — ensure every task has a `cat` field
      // Old records may only have `label` but not `cat`
      if(data.tasks&&data.tasks.length){
        data.tasks=data.tasks.map(t=>({...t,cat:t.cat||t.label||"Study"}));
      }
      setUserData(data);
      if(data.theme)setIsDark(data.theme!=="light");
    }
    return data;
  },[]);

  const loadSessions=useCallback(async(uid)=>{
    const{data}=await supabase.from("sessions").select("*").eq("user_id",uid).order("date",{ascending:false});
    setSessions(data||[]);
    return data||[];
  },[]);
  ...
  useEffect(()=>{
    const restore=async()=>{
      const keep=localStorage.getItem("jyoo_keep");
      const active=sessionStorage.getItem("jyoo_active");
      if(keep!=="1"&&!active){setAuthLoading(false);return;}
      const{data:{session}}=await supabase.auth.getSession();
      if(session?.user){
        setAuthUser(session.user);
        setDataLoading(true);
        const ud=await loadAll(session.user.id);
        setDataLoading(false);
        if(!ud?.tasks?.length)setShowSetup(true);
      }
  ...
  useEffect(()=>{
    ...
    const userCh=supabase.channel("user-rt-"+uid)
      .on("postgres_changes",{event:"*",schema:"public",table:"users",filter:`id=eq.${uid}`},()=>loadUserData(uid))
      .subscribe();
    const sessCh=supabase.channel("sess-rt-"+uid)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"sessions",filter:`user_id=eq.${uid}`},()=>loadSessions(uid))
      .subscribe();
    const iv=setInterval(()=>loadAll(uid),5000);
    return()=>{supabase.removeChannel(userCh);supabase.removeChannel(sessCh);clearInterval(iv);};
  },[authUser,loadAll,loadUserData,loadSessions]);

  // ─── TIMER LOGIC — always mounted, fires handleComplete directly ────────
  // Restore timer from localStorage on app load
  useEffect(()=>{
    ...
    if(elapsed>=saved.totalSecs){
      // Timer already finished while app was closed — fire completion now
      localStorage.removeItem("jyoo_timer");
      const dur=Math.floor(saved.totalSecs/60);
      setTimerDone(true);
      setTimerDuration(dur);
      setTimerTotal(saved.totalSecs);
      setTimerRemaining(0);
      setTimerCategory(saved.category||"Study");
      // Call handleComplete via a small delay so App is fully mounted
      setTimeout(()=>{
        handleCompleteRef.current({
          date:todayStr(),
          start_time:saved.startTime,
          end_time:localTime24(),
          duration:dur,
          category:saved.category||"Study",
        });
      },500);
    }else{
      setTimerTotal(saved.totalSecs);
      setTimerRemaining(saved.totalSecs-elapsed);
      setTimerCategory(saved.category||"Study");
      setTimerDuration(Math.round(saved.totalSecs/60));
  ...
  useEffect(()=>{
    ...
    timerIntervalRef.current=setInterval(()=>{
      const saved=JSON.parse(localStorage.getItem("jyoo_timer")||"null");
      if(!saved){clearInterval(timerIntervalRef.current);setTimerActive(false);return;}
      const elapsed=Math.floor((Date.now()-saved.startTs)/1000);
      ...
      if(rem<=0){
        ...
        localStorage.removeItem("jyoo_timer");
        notifyUser("🌳 Focus Complete!",`+${Math.floor(saved.totalSecs/60)}m of ${saved.category||"Study"} logged! Great work!`);
        setTimerActive(false);
        setTimerDone(true);
        // Fire handleComplete DIRECTLY — no prop chain, no ref indirection
        handleCompleteRef.current({
          date:todayStr(),
          start_time:saved.startTime,
          end_time:localTime24(),
          duration:Math.floor(saved.totalSecs/60),
          category:saved.category||"Study",
        });
      }
    },1000);
  ...
  // Timer actions passed to TimerPage
  const timerSetDuration=(m)=>{
    if(timerActive)return;
    setTimerDuration(m);setTimerTotal(m*60);setTimerRemaining(m*60);
  };
  const timerSetCategory=(c)=>{if(!timerActive)setTimerCategory(c);};
  const notifyUser=(title,body)=>{
    if(!("Notification" in window))return;
    if(Notification.permission==="granted"){
      new Notification(title,{body,icon:"/icon-192.png",badge:"/icon-192.png",tag:"jyoo-timer",renotify:true});
    }
  };
  const timerStart=async()=>{
    // Request permission HERE (user gesture) — this is the only way browsers allow it
    if("Notification" in window && Notification.permission==="default"){
      await Notification.requestPermission();
    }
    const startTime=localTime24();
    timerStartRef.current=startTime;
    localStorage.setItem("jyoo_timer",JSON.stringify({startTs:Date.now(),totalSecs:timerTotal,category:timerCategory,startTime}));
    setTimerActive(true);setTimerDone(false);
  };
  const timerEnd=()=>{
    clearInterval(timerIntervalRef.current);
    const saved=JSON.parse(localStorage.getItem("jyoo_timer")||"null");
    const elapsedSecs=saved?Math.floor((Date.now()-saved.startTs)/1000):(timerTotal-timerRemaining);
    ...
    if(mins>=1){
      ...
      handleCompleteRef.current({
        date:todayStr(),
        start_time:timerStartRef.current||saved?.startTime||localTime24(),
        end_time:localTime24(),
        duration:mins,
        category:saved?.category||timerCategory,
      });
    }else{
      setTimerRemaining(timerTotal);
    }
  };
  const timerReset=()=>{setTimerDone(false);setTimerRemaining(timerTotal);};

  const timerState={remaining:timerRemaining,total:timerTotal,active:timerActive,done:timerDone,category:timerCategory,duration:timerDuration};
  const timerActions={startTimer:timerStart,endTimer:timerEnd,resetTimer:timerReset,setDuration:timerSetDuration,setCategory:timerSetCategory};

  // SESSION SAVE — fetches everything fresh from DB, never uses stale React closures
  const handleComplete=useCallback(async(session)=>{
    if(!session||session.duration<1)return;

    // Get live auth user directly from Supabase, not React state
    let uid,email;
    try{
      ...
      setXpNotif("\u26a0\ufe0f Auth error — please log out and back in");
      setTimeout(()=>setXpNotif(null),8000); return;
    }
    if(!uid){
      setXpNotif("\u26a0\ufe0f Not logged in — please log in again");
      setTimeout(()=>setXpNotif(null),8000); return;
    }

    const cat=session.category||"Study";

    // Fetch current user row fresh from DB (never trust stale closure)
    const{data:freshUser}=await supabase.from("users").select("*").eq("id",uid).single();

    // Ensure users row exists before inserting session (FK guard)
    if(!freshUser){
      const{error:createErr}=await supabase.from("users").insert({
        id:uid,email,username:email.split("@")[0]||"User",
        avatar:"\ud83e\udd8a",theme:"dark",tasks:[],
        total_mins:0,current_streak:0,longest_streak:0,
      });
      if(createErr){
        setXpNotif("\u26a0\ufe0f Account setup failed: "+createErr.message);
        setTimeout(()=>setXpNotif(null),8000); return;
      }
    }

    // Insert session with retries
    let insertOk=false,lastErr=null;
    for(let attempt=1;attempt<=3;attempt++){
      const{error}=await supabase.from("sessions").insert({
        user_id:uid,date:session.date,start_time:session.start_time,
        end_time:session.end_time,duration:session.duration,category:cat,
      });
      if(!error){insertOk=true;break;}
      lastErr=error;
      console.error("[jyoo] session insert attempt "+attempt+":",error.code,error.message);
      if(attempt<3)await new Promise(r=>setTimeout(r,800));
    }
    if(!insertOk){
      setXpNotif("\u26a0\ufe0f Save failed ["+(lastErr?.code||"?")+"]: "+(lastErr?.message||"unknown"));
      setTimeout(()=>setXpNotif(null),10000); return;
    }

    // Recompute totals from fresh session list
    const{data:allSessions}=await supabase.from("sessions").select("*").eq("user_id",uid);
    const allSess=allSessions||[];
    const newTotal=allSess.reduce((a,s)=>a+s.duration,0);
    const streaks=computeStreaks(allSess);

    // UPDATE only the 3 numeric fields — never overwrite tasks/avatar/theme
    await supabase.from("users")
      .update({total_mins:newTotal,current_streak:streaks.current,longest_streak:streaks.longest})
      .eq("id",uid);

    // Log to admin
    try{
      await supabase.from("admin_logs").insert({
        user_id:uid,username:freshUser?.username||email.split("@")[0],email,
        action:"session_complete",
        data:{subject:cat,duration_mins:session.duration,date:session.date,
              start:session.start_time,end:session.end_time,
              streak:streaks.current,best_streak:streaks.longest,
              total_study:fmtStudyTime(newTotal),total_sessions:allSess.length}
      });
    }catch(_){}
    ...
    // Success toast
    const bonus=getDailyChallenge(allSess).done&&!getDailyChallenge(allSess.slice(0,-1)).done?50:0;
    setXpNotif("\u2705 +"+(session.duration+bonus)+" XP \u00b7 "+fmtMins(session.duration)+" saved!"+(bonus?" \ud83c\udfaf+50":""));
    setTimeout(()=>setXpNotif(null),3500);
    // System notification confirming the save (fires after session is in DB)
    if("Notification" in window && Notification.permission==="granted"){
      new Notification("\u2705 Session Saved!",{
        body:"+"+(session.duration+bonus)+" XP \u00b7 "+fmtMins(session.duration)+" of "+(session.category||"Study")+(bonus?" \ud83c\udfaf+50 Bonus!":""),
        icon:"/icon-192.png",tag:"jyoo-save",renotify:true
      });
    }
  },[loadAll]);

  // Ref so timer intervals (which capture nothing) can always call latest handleComplete
  ...
  const handleLogout=async()=>{
    ...
    sessionStorage.removeItem("jyoo_active");localStorage.removeItem("jyoo_keep");
    setForceAuth(true);
  ...
  const handleDeleteAccount=async()=>{
    try{await supabase.from("admin_logs").insert({user_id:authUser.id,username:userData?.username,email:authUser.email,action:"delete_requested",data:{username:userData?.username,email:authUser.email,requested_on:nowLocale(),total_study:fmtStudyTime(userData?.total_mins||0),sessions:sessions.length,streak:userData?.current_streak||0,best_streak:userData?.longest_streak||0,subjects:(userData?.tasks||[]).map(t=>t.label).join(", "),note:"Remove manually: Supabase users table + Authentication tab"}});}catch(_){}
    sessionStorage.removeItem("jyoo_active");localStorage.removeItem("jyoo_keep");localStorage.removeItem("jyoo_timer");
    setForceAuth(true);
    setAuthUser(null);setUserData(null);setSessions([]);setShowSetup(false);setDataLoading(false);
    await supabase.auth.signOut();
    setForceAuth(false);
  ...
  const handleAuth=async(u)=>{
    setForceAuth(false);setDataLoading(true);setAuthUser(u);
    const ud=await loadAll(u.id);
    setDataLoading(false);
    if(!ud?.tasks?.length)setShowSetup(true);
    try{await supabase.from("admin_logs").insert({user_id:u.id,username:ud?.username,email:u.email,action:"login",data:{streak:ud?.current_streak||0,total_study:fmtStudyTime(ud?.total_mins||0),date:todayStr(),time:new Date().toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}});}catch(_){}
  };

  if(forceAuth)return<div style={{background:T.bg,minHeight:"100vh",maxWidth:"480px",margin:"0 auto"}}><style>{CSS}</style><AuthPage onAuth={handleAuth}/></div>;
  if(authLoading||dataLoading)return<LS/>;
  if(!authUser)return<div style={{background:T.bg,minHeight:"100vh",maxWidth:"480px",margin:"0 auto"}}><style>{CSS}</style><AuthPage onAuth={handleAuth}/></div>;
  ...
  if(showSetup)return(
    <div style={{background:T.bg,minHeight:"100vh",maxWidth:"480px",margin:"0 auto",paddingBottom:"30px"}}>
      ...
      <div style={{padding:"18px 16px 0",position:"relative",zIndex:1}}>
        ...
        <p style={{fontSize:"12px",color:T.dim,marginTop:"2px",marginBottom:"14px"}}>Set up your study subjects to get started</p>
      </div>
  ...
  return(
    <div style={{background:T.bg,minHeight:"100vh",maxWidth:"480px",margin:"0 auto",paddingBottom:"72px"}}>
      ...
      {activePage==="timer"&&<TimerPage timerState={timerState} timerActions={timerActions} taskDefs={userData?.tasks||[]} T={T}/>}
      {activePage==="leaderboard"&&<LeaderboardPage currentUserId={authUser.id} T={T} refreshKey={refreshKey}/>}
      {activePage==="stats"&&<StatsPage sessions={sessions} userData={userData} T={T}/>}
      {activePage==="settings"&&<SettingsPage user={authUser} userData={userData} sessions={sessions} onUpdateTasks={handleUpdateTasks} onLogout={handleLogout} onDeleteAccount={handleDeleteAccount} onRefresh={handleRefresh} isDark={isDark} toggleTheme={toggleTheme} T={T}/>}
      {activePage==="admin"&&isAdmin&&<AdminPage T={T} refreshKey={refreshKey}/>}

      {xpNotif&&<XPNotification msg={xpNotif} onDone={()=>setXpNotif(null)}/>}
