...
function computeBadges(sessions,userData){
  ...
  const add=(id)=>{if(!earned_ids.has(id)){earned_ids.add(id);earned.push(BADGE_DEFS.find(b=>b.id===id));}};
  ...
  const hour=(t)=>{if(!t)return 12;return parseInt(t.split(":")[0])||0;};
...
function getDailyChallenge(sessions){
  const today=todayStr();
  const idx=new Date(today).getDate()%CHALLENGES.length;
  const ch=CHALLENGES[idx];
  const todaySessions=sessions.filter(s=>s.date===today);
  const todayMins=todaySessions.reduce((a,s)=>a+s.duration,0);
  let done=false,progress=0,max=ch.target;
  if(ch.type==="duration"){progress=Math.min(todayMins,ch.target);done=todayMins>=ch.target;}
  if(ch.type==="single"){const maxSingle=Math.max(0,...todaySessions.map(s=>s.duration));progress=Math.min(maxSingle,ch.target);done=maxSingle>=ch.target;}
  if(ch.type==="subjects"){const cats=new Set(todaySessions.map(s=>s.category));progress=Math.min(cats.size,ch.target);done=cats.size>=ch.target;}
  if(ch.type==="sessions"){progress=Math.min(todaySessions.length,ch.target);done=todaySessions.length>=ch.target;}
  if(ch.type==="streak"){done=todayMins>=60;progress=done?1:0;}
  return{...ch,done,progress,max};
}
...
function computeAIInsights(sessions){
  ...
  // Best study hour
  const hourMap={};
  sessions.forEach(s=>{const h=hour(s.start_time);hourMap[h]=(hourMap[h]||0)+s.duration;});
  const bestHour=Object.entries(hourMap).sort((a,b)=>b[1]-a[1])[0];
  const bh=parseInt(bestHour?.[0]||12);
  const fmtH=(h)=>h===0?"12 AM":h<12?`${h} AM`:h===12?"12 PM":`${h-12} PM`;
  const bestWindow=`${fmtH(bh)} – ${fmtH(Math.min(23,bh+2))}`;
  // Best day
  const DAYS=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
  const dayMap={};
  sessions.forEach(s=>{const d=new Date(s.date+"T00:00:00").getDay();dayMap[d]=(dayMap[d]||0)+s.duration;});
  const bestDay=Object.entries(dayMap).sort((a,b)=>b[1]-a[1])[0];
  const bestDayName=DAYS[parseInt(bestDay?.[0]||0)];
  // Avg duration
  const avgDur=Math.round(sessions.reduce((a,s)=>a+s.duration,0)/sessions.length);
  // Suggested session (round to nearest 5, nudge +10%)
  ...
  // Consistency score (% of last 14 days with any study)
  ...
  // Focus score (weighted: streak 30%, avg 30%, consistency 40%)
  ...
  // Peak subject
  const subMap={};
  sessions.forEach(s=>{subMap[s.category]=(subMap[s.category]||0)+s.duration;});
...
const makeCSS=(T)=>`
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  @keyframes levelUp{0%{transform:scale(1);}50%{transform:scale(1.2);filter:brightness(1.5);}100%{transform:scale(1);}}
  .xp-bar-fill{animation:xpFill 1s ease forwards;}
  ...
  .xp-notification{position:fixed;top:80px;left:50%;transform:translateX(-50%);z-index:200;background:linear-gradient(135deg,#7c3aed,#a855f7);color:white;padding:10px 20px;border-radius:20px;font-weight:700;font-size:14px;box-shadow:0 4px 24px rgba(168,85,247,0.6);animation:fadeUp 0.4s ease forwards;}
`;
        ...
        <div style={{display:"flex",gap:"10px",alignItems:"flex-start"}}>
          ...
          <div>
            ...
            <p style={{fontSize:"12px",color:T.muted,lineHeight:1.6}}>
              {ins.trend>=10?"Great momentum! You're studying more than last week. Aim for "+ins.suggested+"min sessions during "+ins.bestWindow+" for peak results.":
               ins.trend<=-10?"Your study time dipped this week. Try scheduling a "+ins.suggested+"-min session on "+ins.bestDayName+" to get back on track.":
               "You're consistent! Your optimal window is "+ins.bestWindow+". A "+ins.suggested+"-min session would push your focus score higher."}
            </p>
          </div>
        </div>
      </div>
            ...
            <div key={u.id} className={`lb-row ${isMe?"me":""}`}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:"5px",flexWrap:"wrap"}}>
                  ...
                  <span className="level-badge" style={{fontSize:"9px"}}>{computeLevel(u.total_mins||0).icon} Lv.{computeLevel(u.total_mins||0).level}</span>
                </div>
                <div style={{fontSize:"11px",color:T.dim,marginTop:"2px"}}>🔥 {u.current_streak||0} · 🏆 {u.longest_streak||0}</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div className="mono" style={{fontSize:"15px",fontWeight:700,color:isMe?"#c084fc":"#a855f7"}}>{tab==="level"?`Lv.${computeLevel(u.total_mins||0).level}`:val}</div>
                <div style={{fontSize:"10px",color:T.dim}}>{tab==="time"?"studied":tab==="level"?computeLevel(u.total_mins||0).name:"streak"}</div>
              </div>
            </div>
          );
        })
    ...
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      <div className="card" style={{padding:"16px",marginBottom:"12px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px"}}>
          ...
          {(()=>{const lv=computeLevel(userData?.total_mins||0);return(
            <div style={{textAlign:"right"}}>
        ...
        <div style={{height:"5px",background:"rgba(168,85,247,0.15)",borderRadius:"3px",marginBottom:"12px"}}>
          <div style={{height:"100%",width:`${computeLevel(userData?.total_mins||0).pct}%`,background:"linear-gradient(90deg,#7c3aed,#a855f7)",borderRadius:"3px"}}/>
        </div>
        <div style={{display:"flex",gap:"8px",marginBottom:"12px"}}>
          {[
            {icon:"⚡",label:"Total XP",val:`${userData?.total_mins||0}`},
            {icon:"🏅",label:"Badges",val:`${computeBadges(sessions||[],userData).length}/${BADGE_DEFS.length}`},
            {icon:"🔥",label:"Streak",val:`${userData?.current_streak||0}d`},
          ].map(s=>(
        ...
        <button className="btn-ghost" onClick={()=>setShowBadges(p=>!p)} style={{width:"100%",padding:"9px",fontSize:"12px"}}>
      ...
      <div className="card" style={{padding:"14px",marginBottom:"12px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"7px"}}>
          {[
            {icon:"🔐",label:"JWT Auth",status:"Active",ok:true},
            {icon:"🌐",label:"HTTPS",status:"Enforced",ok:true},
            {icon:"🗄️",label:"RLS Policies",status:"Enabled",ok:true},
            {icon:"🔑",label:"Password Hash",status:"Supabase bcrypt",ok:true},
          ].map(s=>(
            <div key={s.label} style={{display:"flex",align:"center",gap:"6px",padding:"7px 9px",background:s.ok?"rgba(16,185,129,0.06)":"rgba(239,68,68,0.06)",border:`1px solid ${s.ok?"rgba(16,185,129,0.2)":"rgba(239,68,68,0.2)"}`,borderRadius:"8px"}}>
              <span style={{fontSize:"14px"}}>{s.icon}</span>
              <div>
                <div style={{fontSize:"10px",fontWeight:700,color:T.text}}>{s.label}</div>
                <div style={{fontSize:"9px",color:s.ok?"#10b981":"#ef4444"}}>{s.status}</div>
              </div>
            </div>
          ))}
        </div>
...
function AdminUserDetail({user:u,logs,sessions,T,onBack}){
  ...
  return(
    <div style={{padding:"20px 16px",position:"relative",zIndex:1}}>
      ...
      {/* Level Badge */}
      {(()=>{const lv=computeLevel(u.total_mins||0);return(<div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px",padding:"10px 12px",background:"rgba(168,85,247,0.06)",borderRadius:"10px",border:"1px solid rgba(168,85,247,0.15)"}}>
        <span style={{fontSize:"22px"}}>{lv.icon}</span>
        <div style={{flex:1}}><div style={{fontWeight:700,fontSize:"13px",color:"#c084fc"}}>Level {lv.level} · {lv.name}</div><div style={{height:"4px",background:"rgba(168,85,247,0.15)",borderRadius:"2px",marginTop:"5px"}}><div style={{height:"100%",width:`${lv.pct}%`,background:"linear-gradient(90deg,#7c3aed,#a855f7)",borderRadius:"2px"}}/></div></div>
                ...
                <div key={l.id} style={{padding:"8px 10px",background:"rgba(239,68,68,0.1)",borderRadius:"8px",marginBottom:"6px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"3px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                      ...
                      <button onClick={async()=>{await supabase.from("admin_logs").insert({user_id:l.user_id,username:l.username,email:l.email,action:"deleted",data:{...l.data,dismissed_by:"admin",deleted_at:new Date().toLocaleString("en-IN",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}});await fetchAll();}} style={{background:"rgba(239,68,68,0.2)",border:"1px solid rgba(239,68,68,0.4)",borderRadius:"50%",width:"20px",height:"20px",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#f87171",fontSize:"11px",fontWeight:700,lineHeight:1,padding:0,flexShrink:0}}>✕</button>
                    </div>
                  </div>
              ...
              <div style={{background:"#1a0a2e",borderRadius:"16px",padding:"24px",width:"100%",maxWidth:"360px",border:"1px solid rgba(16,185,129,0.3)"}}>
                ...
                <div style={{display:"flex",gap:"10px"}}>
                  ...
                  <button onClick={async()=>{
                    const doneAt=new Date().toLocaleString("en-IN",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"});
                    await supabase.from("admin_logs").insert({user_id:confirmDone.user_id,username:confirmDone.username,email:confirmDone.email,action:"deleted",data:{...confirmDone.data,deleted_by:"admin",deleted_at:doneAt}});
                    setConfirmDone(null);fetchAll();
                  }} style={{flex:1,padding:"11px",borderRadius:"10px",background:"rgba(16,185,129,0.2)",border:"1px solid rgba(16,185,129,0.4)",color:"#10b981",fontFamily:"Space Grotesk,sans-serif",fontWeight:700,cursor:"pointer",fontSize:"13px"}}>Yes, Done ✓</button>
                </div>
              </div>
            </div>
          )}
...
export default function App(){
  ...
  const handleComplete=useCallback(async(session)=>{
    ...
    // STEP 1: Insert session — try up to 3 times
    let ins=null;
    for(let attempt=0;attempt<3;attempt++){
      const {data:d,error:e}=await supabase.from("sessions").insert({user_id:uid,...session}).select().single();
      if(d&&!e){ins=d;break;}
      if(attempt<2)await new Promise(r=>setTimeout(r,600));
    }
    if(!ins){
      // Last resort: insert without select
      await supabase.from("sessions").insert({user_id:uid,...session});
    }

    // STEP 2: Fetch ALL sessions fresh
    ...
    // STEP 4: Save to users — try UPDATE first, then UPSERT, guaranteed to work for all users
    ...
    // STEP 5: Log to admin
    try{await supabase.from("admin_logs").insert({user_id:uid,username:userData?.username,email:authUser.email,action:"session_complete",data:{subject:session.category,duration_mins:session.duration,date:session.date,start:session.start_time,end:session.end_time,streak:streaks.current,best_streak:streaks.longest,total_study:fmtStudyTime(newTotal),total_sessions:freshSessions.length}});}catch(_){}

    // STEP 6: Force reload ALL data so every page is fresh
    await loadAll(uid);
    setRefreshKey(k=>k+1);

    // STEP 7: XP toast
    const xpEarned=session.duration;
    const prevSessions=freshSessions.slice(0,-1);
    const bonus=getDailyChallenge(freshSessions).done&&!getDailyChallenge(prevSessions).done?50:0;
    setXpNotif(`+${xpEarned+bonus} XP${bonus>0?" (+50 Challenge Bonus!)":""} · ${fmtMins(xpEarned)} logged`);
